...
typedef struct {
    yy_png_chunk_IHDR header;   
    yy_png_chunk_info *chunks;      
    uint32_t chunk_num;          
    yy_png_frame_info *apng_frames; 
...
static bool yy_png_validate_animation_chunk_order(yy_png_chunk_info *chunks,  
                                                  uint32_t chunk_num,         
...
static BOOL YYCGImageDecodeToBitmapBufferWithAnyFormat(CGImageRef srcImage, vImage_Buffer *dest, vImage_CGImageFormat *destFormat) {
    ...
    srcData = srcProvider ? CGDataProviderCopyData(srcProvider) : NULL; 
...
YYImageType YYImageDetectType(CFDataRef data) {
    ...
    switch (magic4) {
        case YY_FOUR_CC(0x4D, 0x4D, 0x00, 0x2A): { 
            return YYImageTypeTIFF;
        } break;
        case YY_FOUR_CC(0x49, 0x49, 0x2A, 0x00): { 
            return YYImageTypeTIFF;
        } break;
        case YY_FOUR_CC(0x00, 0x00, 0x01, 0x00): { 
            return YYImageTypeICO;
        } break;
        case YY_FOUR_CC(0x00, 0x00, 0x02, 0x00): { 
            return YYImageTypeICO;
        } break;
        case YY_FOUR_CC('i', 'c', 'n', 's'): { 
            return YYImageTypeICNS;
        } break;
        case YY_FOUR_CC('G', 'I', 'F', '8'): { 
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < info->chunk_num; i++) {
        ...
        if (!insertBefore && chunk->fourcc == YY_FOUR_CC('I', 'D', 'A', 'T')) {
            ...
            acTL[1] = YY_FOUR_CC('a', 'c', 'T', 'L'); 
            acTL[2] = yy_swap_endian_uint32((uint32_t)pngDatas.count); 
            acTL[3] = yy_swap_endian_uint32((uint32_t)_loopCount); 
            acTL[4] = yy_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(acTL + 1), 12)); 
            ...
            uint8_t fcTL[38] = {0};
            *((uint32_t *)fcTL) = yy_swap_endian_uint32(26); 
            *((uint32_t *)(fcTL + 4)) = YY_FOUR_CC('f', 'c', 'T', 'L'); 
                ...
                *((uint32_t *)(fcTL + 34)) = yy_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(fcTL + 4), 30));
                ...
                for (int d = 0; d < frame->chunk_num; d++) {
                    ...
                    if (dchunk->fourcc == YY_FOUR_CC('I', 'D', 'A', 'T')) {
                        ...
                        [result appendBytes:&fourcc length:4]; 
                        uint32_t sq = yy_swap_endian_uint32(apngSequenceIndex);
                        [result appendBytes:&sq length:4]; 
                        [result appendBytes:(((uint8_t *)frameData.bytes) + dchunk->offset + 8) length:dchunk->length]; 
